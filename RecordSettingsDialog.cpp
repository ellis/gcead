...
RecordSettingsDialog::RecordSettingsDialog(Idac* idac, bool bSendChanges, QWidget *parent)
	: QDialog(parent)
{
	ui.setupUi(this);
	...
	if (idac != NULL)
	{
		...
		{
			QString s;
			int nFracOfVolt = 1000000 / n;
			if (nFracOfVolt < 1000)
				s = tr("%0 mV").arg(1000.0 / nFracOfVolt, 0, 'f', 1);
			else
				s = tr("%0 �V").arg(1000000.0 / nFracOfVolt, 0, 'f', 1);
			ranges << s;
		}
	}
	...
	ui.chkInvert_1->setChecked(chan->bInvert);
	...
	ui.chkInvert_2->setChecked(chan->bInvert);
	...
	ui.chkInvert_3->setChecked(settings->nDigitalInversionMask & 0x01);

	ui.chkEnabled_4->setChecked(settings->nDigitalEnabledMask & 0x02);
	ui.chkInvert_4->setChecked(settings->nDigitalInversionMask & 0x02);
...
void RecordSettingsDialog::on_cmbHighpass_activated(int iChan, int i)
{
	...
	chan->iHighpass = i;
	if (m_idac != NULL)
		m_idac->resendChannelSettings(iChan);
...
void RecordSettingsDialog::on_chkInvert_3_clicked()
{
	if (ui.chkInvert_3->isChecked())
		Globals->idacSettings()->nDigitalInversionMask |= 0x01;
	else
		Globals->idacSettings()->nDigitalInversionMask &= ~0x01;
}
...
void RecordSettingsDialog::on_chkEnabled_4_clicked()
{
	if (ui.chkEnabled_4->isChecked())
		Globals->idacSettings()->nDigitalEnabledMask |= 0x02;
	else
		Globals->idacSettings()->nDigitalEnabledMask &= ~0x02;
}
...
void RecordSettingsDialog::on_chkInvert_4_clicked()
{
	if (ui.chkInvert_4->isChecked())
		Globals->idacSettings()->nDigitalInversionMask |= 0x02;
	else
		Globals->idacSettings()->nDigitalInversionMask &= ~0x02;
}
