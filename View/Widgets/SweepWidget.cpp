...
void SweepWidget::setDivisionCount(int nDivisions)
{
...
void SweepWidget::paintEvent(QPaintEvent* e)
{
	...
	if (m_nDivisions > 1)
	{
		QPen penGrid = QPen(Qt::gray, 0, Qt::DotLine);
		p.setPen(penGrid);

		double yInc = double(height()) / m_nDivisions;
		double y = yInc;
		for (int i = 0; i < m_nDivisions - 1; i++)
		{
