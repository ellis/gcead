...
MainScope::MainScope(MainScopeUi* ui, Idac* idac, QObject* parent)
	: QObject(parent)
{
	Q_ASSERT(ui != NULL);
	...
	connect(m_actions->recordOptions, SIGNAL(triggered()), this, SLOT(on_actions_recordOptions_triggered()));
...
void MainScope::updateActions()
{
	...
	bool bIdac = (m_idac != NULL && bHaveFile);
	m_actions->recordRecord->setEnabled(bIdac && !m_bRecording);
	m_actions->recordOptions->setEnabled(bIdac && !m_bRecording);
...
void MainScope::on_actions_recordOptions_triggered()
{
